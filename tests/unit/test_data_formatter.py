...
def build_rows(settings: ExportSettings, table: PriceTable, difference) -> list[list]:
    ws = Workbook().active
...
def test_percentage_formatting_is_relative(settings: ExportSettings, table: PriceTable) -> None:
...
def test_autofilter_spans_the_data_block(settings: ExportSettings, table: PriceTable) -> None:
    ws = Workbook().active
...
def test_difference_columns_are_styled_beyond_column_z(pharmacy_count: int, settings: ExportSettings) -> None:
    """Regression for B10.
    ...
    for column in range(4, total_columns + 1, 2):
        ...
        assert ws.column_dimensions[letter].width == settings.diff_width, letter
...
def test_wide_table_autofilter_uses_the_real_last_column(settings: ExportSettings) -> None:
    ws = Workbook().active
