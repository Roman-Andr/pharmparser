...
class Macro(ABC):  # noqa: B024  # TODO(docs/REFACTOR_PLAN.md phase 3): subclasses set
    # `code` in __init__ before position codes exist (bug B1); the rewrite makes `code`
    # an abstract property rendered on demand, which gives this ABC a real abstract member.
    start_col = 4
    start_row = 3
    end_row = 100000
...
class SortMacro(Macro):
    def __init__(self, column, end_column, sort_order, sheet_name):
        ...
        sort_stmt = (
            ...
            f'Order1:={sort_order.value}, Header:=xlYes'
        )
        self.code = f"""
