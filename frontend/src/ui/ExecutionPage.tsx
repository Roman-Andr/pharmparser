...
export function ExecutionPage() {
  ...
  return <section><header className="page-title"><div><p className="eyebrow">ЗАПУСК</p><h1>{done ? "Выполнение завершено" : "Формируем отчет"}</h1><p>Каждая аптека загружается независимо. Окно можно оставить в фоне.</p></div></header>
    ...
    <div className="pharmacy-grid">{[...pharmacies.values()].map((event) => <article className="card" key={event.pharmacy_id}><span className={`status ${event.stage === "Ошибка" ? "bad" : ""}`}>{event.stage}</span><h3>{event.message.replace(/^[^:]+:\s*/, "")}</h3><p className={event.detail ? "error" : ""}>{event.detail || (event.current != null ? `${event.current} позиций` : "Ожидание данных")}</p></article>)}</div>
